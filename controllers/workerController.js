const Worker = require("../models/workerModel")
const Otp = require("../models/otpModel")
const otpGenerator = require("otp-generator")
const bcrypt = require("bcrypt")
const saltRounds = 10
const sendToken = require("../utils/jwtToken");
const catchAsyncErrors = require("../middleware/catchAsyncErrors")
const cloudinary = require("cloudinary")
const saveUser = require("../utils/saveUser")

exports.sendOtp = catchAsyncErrors(async function (req, res) {
    const { name, contact, password, longitude, latitude, dob } = req.body;
    console.log(req.body)
    const location = [Number(longitude), Number(latitude)]
    const worker = await Worker.findOne({
        contact: contact
    })
    if (worker) return res.status(409).send("Worker already registered!")
    const OTP = otpGenerator.generate(6, {
        digits: true, lowerCaseAlphabets: false, upperCaseAlphabets: false, specialChars: false
    })
    const otp = await Otp.create({ contact: contact, otp: OTP, name, password, location, dob });
    return res.status(200).send("Otp send successfully!" + otp);
})

exports.registerWorker = catchAsyncErrors(async function (req, res) {
    const { contact, otp } = req.body

    const otpHolder = await Otp.find({
        number: contact
    })

    if (otpHolder.length === 0) return res.status(410).send("You use an Expired OTP!");

    const rightOtpFind = otpHolder[otpHolder.length - 1];
    const validUser = (otp === rightOtpFind.otp)

    // Hashing and salting password
    bcrypt.hash(rightOtpFind.password, saltRounds, async function (err, hash) {
        if (rightOtpFind.contact === contact && validUser) {
            const createdWorker = await Worker.create({
                name: rightOtpFind.name,
                contact: rightOtpFind.contact,
                password: hash,
                location: rightOtpFind.location,
                birthday: rightOtpFind.dob
            })
            await Otp.deleteMany({
                contact: rightOtpFind.contact
            });
            sendToken(createdWorker, 200, res, 1);
        } else {
            return res.status(400).send("Your OTP was wrong!")
        }
    })
})

exports.loginWorker = catchAsyncErrors(async function (req, res) {
    const contact = req.body.contact
    const password = req.body.password

    Worker.findOne({ contact: contact }, async function (err, foundWorker) {
        if (err) {
            res.status(401).send("Incorrect Contact or Password")
        }
        else {
            if (foundWorker) {
                const isPasswordMatched = await foundWorker.comparePassword(password);
                if (isPasswordMatched) {
                    sendToken(foundWorker, 200, res, 1)
                }
                else res.status(401).send("Incorrect Contact or Password")
            }
            else res.status(401).send("Incorrect Contact or Password")
        }
    })
})

exports.logoutWorker = catchAsyncErrors(async function (req, res) {
    res.cookie("token", null, {
        expires: new Date(Date.now()),
        httpOnly: true
    })

    res.status(200).json({
        success: true,
        message: "Logged Out Successfully"
    })
})

exports.getWorkerDetails = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    res.status(200).json({
        success: true,
        user
    })
})

exports.updateName = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    user.name = req.body.name
    saveUser(user, res, 1)
})

exports.updateEmail = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    user.email = req.body.email
    saveUser(user, res, 1)
})

exports.updatePassword = catchAsyncErrors(async function (req, res) {
    const newPassword = req.body.newPassword
    const password = req.body.password
    const user = await Worker.findById(req.worker.id)
    console.log(user)
    const isPasswordMatched = await user.comparePassword(password);
    if (isPasswordMatched) {
        bcrypt.hash(newPassword, saltRounds, function (err, hash) {
            user.password = hash
            user.save().then((u) => {
                res.status(200).json({
                    success: true,
                })
            }).catch((err) => {
                console.log(err)
                res.status(400).json({
                    success: false,
                })
            })
        })
    }
    else res.status(401).send("Incorrect Password.")
})

exports.updateWorkerAddress = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    user.address = req.body
    saveUser(user, res, 1)
})

exports.updateWorkerAadhar = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    user.aadhar = req.body.aadhar
    saveUser(user, res, 1)
})

exports.updateWorkerLocation = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    const location = [Number(req.body.longitude), Number(req.body.latitude)]
    user.location = location
    saveUser(user, res, 1)
})

exports.updateWorkerContact = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    const { newContact } = req.body
    const contact = await Worker.findOne({
        contact: newContact
    })
    if (contact) res.status(409).send({
        message: "Mobile number already exists."
    })
    const OTP = otpGenerator.generate(6, {
        digits: true, lowerCaseAlphabets: false, upperCaseAlphabets: false, specialChars: false
    })
    const otp = await Otp.create({ contact: newContact, otp: OTP, name: user.name, password: user.password, location: user.location });
    return res.status(200).send("Otp send successfully!" + otp);
})

exports.updateWorkerContactVerify = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    const { newContact, otp } = req.body
    const otpHolder = await Otp.find({
        number: newContact
    })

    if (otpHolder.length === 0) return res.status(410).send("You use an Expired OTP!");

    const rightOtpFind = otpHolder[otpHolder.length - 1];
    const validUser = (otp === rightOtpFind.otp)

    if (rightOtpFind.contact === newContact && validUser) {
        user.contact = newContact
        user.save()
        await Otp.deleteMany({
            contact: rightOtpFind.contact
        });
        res.status(200).send({
            success: "true",
            user
        })
    }
    else res.status(400).send({
        message: "Your OTP was wrong."
    })
})

exports.updateWorkerProfile = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    const oldImage = user.profilepic.public_id
    const myCloud = await cloudinary.v2.uploader.upload(req.files.avatar.tempFilePath, {
        folder: "Domestica/worker",
        width: 400,
        crop: "scale",
    });
    const profilepic = {
        public_id: myCloud.public_id,
        url: myCloud.secure_url
    }
    user.profilepic = profilepic
    saveUser(user, res, 1)
    cloudinary.v2.uploader
        .destroy(oldImage)
        .then(result => console.log(result))
        .catch(err => console.log(err));
})

exports.updateDob = catchAsyncErrors(async function (req, res) {
    const user = await Worker.findById(req.worker.id)
    user.birthday = req.body.dob
    saveUser(user, res, 1)
})